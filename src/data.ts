...
export enum DataEventType { StateChanged = 0, DataChanged = 1, CursorMoved = 2, Refreshed = 3 };
...
export class BaseDataLink<T extends IRecordSource | IRecordSetSource> implements IDataLink {
    ...
    public onChange(eventType: DataEventType, field?: string): void {
        if (this.onChangeEvent)
            this.onChangeEvent(eventType, field);
    }
...
export class FieldDataLink extends BaseDataLink<IRecordSource> {
    ...
    constructor(onChangeEvent: IOnDataChangeEvent, converter?: IValueConverter) {
        super(onChangeEvent);
        this.converter = converter;
    }

    public get dataField() { return this._dataField; }
    public set dataField(dataField: string) {
        if (this._dataField != dataField) {
            this._dataField = dataField;
            if (this.dataSource)
                this.onChange(DataEventType.Refreshed, dataField);
        }
    }
...
export class LookupDataLink extends RecordSetDataLink {
    ...
    public set keyField(keyField: string) {
        if (this._keyField != keyField) {
            this._keyField = keyField;
            if (this.dataSource)
                this.onChange(DataEventType.Refreshed, keyField);
        }
    }
    public get displayField() { return this._displayField; }
    public set displayField(displayField: string) {
        if (this._displayField != displayField) {
            this._displayField = displayField;
            if (this.dataSource)
                this.onChange(DataEventType.Refreshed, displayField);
        }
    }
...
export class BaseSource implements IDataSource {
    ...
    public notifyLinks(eventType: DataEventType, field?: string): void {
        for (let i = 0; i < this._links.length; i++) {
            this._links[i].onChange(eventType, field);
        }
    }
}
...
export class RecordSource extends BaseSource implements IRecordSource {
    ...
    protected setState(value: RecordState): void {
        if (this._state != value) {
            this._state = value;
            this.notifyLinks(DataEventType.StateChanged);
        }
    }
}
